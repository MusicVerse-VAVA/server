...
public class SongsByAlbum extends POSTRequestHandler {
    ...
    private static final String loadSongsByRegex = Util.loadResource("/com/musicverse/server/sql/load_songs_by_regex.sql");
    ...
    public boolean handlePostRequest(String url, String params, HttpExchange exchange) throws Throwable {
        if (!"/songsByAlbum".equals(url) && !"/songsByRegex".equals(url)) return false;
        val request = HttpHelper.parseJSON(exchange);

        val songs = db.query("/songsByAlbum".equals(url) ? loadSongsByAlbum : loadSongsByRegex,
                    (ps) -> {
                        if ("/songsByAlbum".equals(url)) {
                            ps.setInt(1, request.getInt("album_id"));
                        } else {
                            ps.setString(1, request.getString("regex"));
                        }
                    },
                    (rs) -> {
        ...
        /*else {
            ...
            val songs = db.query(loadSongsByRegex,
                    (ps) -> {
